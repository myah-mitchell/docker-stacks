...
`compose.yaml` publishes the HTTPS web console directly on the host (`53443:53443`) in addition to routing it through Traefik with `chain-authentik@file` SSO. The direct port bypasses that SSO gate entirely, so access via `:53443` relies solely on Technitium's own login.
