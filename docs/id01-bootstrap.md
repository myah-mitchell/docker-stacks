...
Then follow [How to deploy it](traefik-bootstrap.md#how-to-deploy-it) for the Stack resource. Set its target *Server* to **id01**. Set `SERVER_NAME` to `id01`. Use the same `SUB_DOMAIN_NAME` and `DOMAIN_NAME` you will use in step 5.
...
mailrise lands on ci01 later and is a plausible relay for this. See [What still lands on ci01](ci01-bootstrap.md#what-still-lands-on-ci01).
