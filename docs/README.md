...
`km01` is the one deliberate exception, provisioned and started entirely by hand, because Komodo cannot GitOps-deploy itself the first time. See [`komodo-bootstrap.md`](komodo-bootstrap.md).
...
| --- | --- | --- | --- | --- |
| 1 | `km01` | Komodo GitOps engine | [`komodo-bootstrap.md`](komodo-bootstrap.md) | Up and healthy |
| 2 | `ci01` | Semaphore first, rest of core-infra later | [`ci01-bootstrap.md`](ci01-bootstrap.md) | In progress |
...
Until then, deploy [`traefik-bootstrap.md`](traefik-bootstrap.md) on that VM. It is real Traefik routing on real hostnames, with self-signed TLS and `chain-no-auth@file` instead. Set the stack's `TRAEFIK_AUTH_CHAIN` to `chain-no-auth@file` when you deploy it. [`ci01-bootstrap.md`](ci01-bootstrap.md) step 9 works through it for the first real case.
...
Steps 1 to 4 of [`ci01-bootstrap.md`](ci01-bootstrap.md) are identical for every VM. Step 5's onboarding key is required for every future host. Step 9's `traefik-bootstrap` deploy applies until `system-agent` replaces it fleet-wide. Only the stack-specific steps after that differ.

## The rest of the docs

| Page | What it covers |
| --- | --- |
| [`conventions.md`](conventions.md) | Naming and secrets rules every other page assumes |
| [`komodo-bootstrap.md`](komodo-bootstrap.md) | `km01`, the one host built by hand |
| [`ci01-bootstrap.md`](ci01-bootstrap.md) | `ci01` and Semaphore, the template for every VM after it |
| [`semaphore-setup.md`](semaphore-setup.md) | Wiring Semaphore to the `ansible` repo and pushing the first real secret |
| [`traefik-bootstrap.md`](traefik-bootstrap.md) | The temporary per-VM Traefik used before `pk01` and `id01` exist |
| [`stacks.md`](stacks.md) | What every stack in this repo deploys, independent of bootstrap order |
