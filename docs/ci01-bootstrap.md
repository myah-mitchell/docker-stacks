...
task) once you point a Stack resource at it in step 10. These folders have to exist
...
## 9. Generate Semaphore's secrets
...
10, as resource-level variable overrides — **never commit real values for these into
`komodo.env`.**

## 10. Create the Stack resource for `stacks/semaphore-server`
...
4. Under **Environment**, set the source to the committed
   ...
   - `SEMAPHORE_COOKIE_HASH` / `SEMAPHORE_COOKIE_ENCRYPTION` /
     `SEMAPHORE_ACCESS_KEY_ENCRYPTION` — the three values from step 9.
...
Semaphore has no direct published port like Komodo's `:9120` fallback, and its
Traefik labels gate it behind `chain-authentik@file` — neither Traefik nor Authentik
exist yet at this point in the plan, so there's no "normal" way to reach its UI yet.
Reach it directly on the Docker network instead, via an SSH tunnel to the container's
own IP:

```bash
# on ci01:
docker inspect -f '{{.NetworkSettings.Networks.semaphore_frontend.IPAddress}}' semaphore-semaphore
```

Then from your workstation:

```bash
ssh -L 3000:<container-ip-from-above>:3000 <you>@<ci-ip>
```

and open `http://localhost:3000`. This same trick — SSH-tunnel straight to a
container's own IP on its Docker network — is the general fallback for reaching
*any* stack's UI before `id01`/Authentik exists, not just Semaphore's; most stacks in
this plan are gated behind `chain-authentik@file` the same way.

## 13. Post-deploy: wire Semaphore to the `ansible` repo
...
needed once Semaphore's Template from step 13 above is in place; steps 8–11
registering/deploying its specific stack will differ).
