...
Generated files per stack:
  ...
  - README.md   : Merged from existing/base README + each container's 
                  stack-README.md and setup.yaml
  - .env        : Testing file derived from the generated komodo.env
  - setup.yaml  : Every container's setup.yaml, filtered to the stack's
                  services and merged, for the ansible stacks role
...
def build_readme(base_content, existing_content, containers_dir,
    ...
    Merge rules:
      ...
      7. A container's setup.yaml is rendered into folder and firewall
         sections placed ahead of its hand-written stack-README.md ones.
    ...
    for name in container_names:
        ...
        if not readme_path.exists() and setup is None:
            continue
        content = ''
        ...
        if setup is not None:
            setup = filter_setup(setup, active_tags)
            sections = merge_setup_sections(
                sections, build_setup_sections(name, setup)
            )
...
# endregion
# ============================================================================
# region Host Setup: setup.yaml -> README Sections
# ============================================================================

SETUP_H1 = 'Create and Setup Required Folders'
SETUP_ROOT_DIRS = {
    'volumes': '/opt/docker/volumes/$projectName',
    'logs': '/opt/docker/logs/$projectName',
}
SETUP_RAW_URL = 'https://raw.githubusercontent.com/myah-mitchell/docker-stacks/main'
SETUP_STACK_HEADER = (
    "# Generated by scripts/build.py from each container's setup.yaml. Do not edit.\n"
    "# The ansible stacks role applies this file. See scripts/project-layout.md.\n"
)

# Per list: (required keys, optional keys). The ansible stacks role reads the
# generated files without validating them, so this is where the schema is enforced.
SETUP_SCHEMA = {
    'folders': ({'path', 'owner', 'group'}, {'mode', 'root', 'services'}),
    'files': ({'path', 'source', 'owner', 'group'}, {'mode', 'services'}),
...
def load_setup(containers_dir, name):
    ...
    setup.yaml lists what a host needs before the container's first deploy:
    folders under ${DOCKER_VOLUMES}/<projectName>/, config files seeded from
    ...
    """
    path = containers_dir / name / 'setup.yaml'
    if not path.exists():
        return None
    ...
    try:
        ...
        fail("reading it needs PyYAML (pip install pyyaml)")
    ...
    for key, (required, optional) in SETUP_SCHEMA.items():
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                fail(f"each '{key}' entry must be a mapping")
            missing = required - set(entry)
            if missing:
                fail(f"a '{key}' entry is missing {', '.join(sorted(missing))}")
            extra = set(entry) - required - optional
            if extra:
                fail(f"a '{key}' entry has unknown keys {', '.join(sorted(extra))}")
            services = entry.get('services')
            if services is not None and not (
                    isinstance(services, list) and services
                    and all(isinstance(s, str) for s in services)):
    ...
    for folder in data['folders']:
        ...
        if folder['root'] not in SETUP_ROOT_DIRS:
            fail(f"root of '{folder['path']}' must be volumes or logs")

    for entry in data['folders'] + data['files']:
        rel = PurePosixPath(str(entry['path']))
        if rel.is_absolute() or '..' in rel.parts:
            fail(f"path '{rel}' must be relative, with no '..'")
        for id_key in ('owner', 'group'):
            if not isinstance(entry[id_key], int):
                fail(f"{id_key} of '{rel}' must be a numeric host ID")
        mode = entry.get('mode')
        if mode is not None and not (
                isinstance(mode, str) and re.fullmatch(r'0[0-7]{3}', mode)):
            fail(f"mode of '{rel}' must be a quoted octal string such as \"0700\"")

    # Files always land under the volumes root, never logs.
    folder_paths = {
        str(entry['path']) for entry in data['folders']
        if entry['root'] == 'volumes'
    }
    ...
    for rule in data['firewall']:
        if not isinstance(rule['port'], int):
            fail("firewall port must be a number")
        if rule['proto'] not in ('tcp', 'udp'):
            fail("firewall proto must be tcp or udp")
        if rule['allow_from'] not in ('internal', 'any'):
            fail("firewall allow_from must be internal or any")

    return data


def filter_setup(setup, active_tags):
    """Drop setup entries whose services are all absent from the stack.

    An entry with no 'services' key applies to every variant. With no
    active tags at all, nothing is dropped.

    Args:
        setup:       Validated dict from load_setup().
        active_tags: Set of the container's variant names used by the stack.

    Returns:
        New dict with the same three lists, filtered.
    """
    def keep(entry):
        services = entry.get('services')
        return not services or not active_tags or bool(set(services) & active_tags)

    return {key: [e for e in setup[key] if keep(e)] for key in SETUP_SCHEMA}


def collect_stack_setup(containers_dir, container_names, service_map):
    """Merge every container's filtered setup.yaml into one stack-wide dict.

    Two containers may list the same folder (postgres-data, for one) or the
    same port. Identical entries collapse into one; entries that share a
    path but differ in owner, group, mode, or source stop the build.
    ...
    Returns:
        Dict with 'folders', 'files', and 'firewall' lists, each entry
        stripped of 'services'.
    """
    merged = {key: {} for key in SETUP_SCHEMA}
    identity = {
        'folders': lambda e: (e['root'], str(e['path'])),
        'files': lambda e: str(e['path']),
        'firewall': lambda e: (e['port'], e['proto'], e['allow_from']),
    }
    for name in container_names:
        setup = load_setup(containers_dir, name)
        if setup is None:
            continue
        setup = filter_setup(setup, service_map.get(name, set()))
        for key in SETUP_SCHEMA:
            for entry in setup[key]:
                entry = {k: v for k, v in entry.items() if k != 'services'}
                ident = identity[key](entry)
                existing = merged[key].get(ident)
                if existing is None:
                    merged[key][ident] = entry
                elif key != 'firewall' and existing != entry:
                    print(f"Error: containers/{name}/setup.yaml: '{entry['path']}' "
                          f"conflicts with the same path from another container")
                    sys.exit(1)
...
def render_stack_setup(project_name, setup):
    ...
    PyYAML versions. Strings are JSON-quoted, which YAML reads unchanged.
    ...
    Returns:
        The file content, ending in a newline.
    """
    order = {
        'folders': ('root', 'path', 'owner', 'group', 'mode'),
        ...
        for entry in setup[key]:
            ...
            for field in fields:
                if field not in entry:
                    continue
                value = entry[field]
                text = str(value) if isinstance(value, int) else json.dumps(str(value))
                lines.append(f"{prefix}{field}: {text}")
                prefix = '    '
    return '\n'.join(lines) + '\n'


def build_setup_sections(name, setup):
    """Render a container's setup.yaml as stack-README.md sections.

    Produces the same shape a hand-written stack-README.md uses: an H1 of
    SETUP_H1 holding a folders H2 and, when there are rules, a firewall H2.
    ...
    Returns:
        List holding one H1 Section.
    """
    note = "Generated from `setup.yaml`, which the ansible `stacks` role also applies."
    h1 = Section(heading_line=f'# {SETUP_H1}', heading_text=SETUP_H1, level=1)

    if setup['folders'] or setup['files']:
        lines = ['', note, '', '```bash']
        for folder in setup['folders']:
            dest = f"{SETUP_ROOT_DIRS[folder['root']]}/{folder['path']}"
            lines.append(f"mkdir -p {dest}")
            lines.append(f"sudo chown {folder['owner']}:{folder['group']} {dest}")
            if folder.get('mode'):
                lines.append(f"sudo chmod {folder['mode'][1:]} {dest}")
        for item in setup['files']:
            dest = f"{SETUP_ROOT_DIRS['volumes']}/{item['path']}"
            lines.append(f"sudo test -e {dest} \\")
            lines.append(f"  || sudo curl -fsSL -o {dest} \\")
            lines.append(f"  {SETUP_RAW_URL}/{item['source']}")
            lines.append(f"sudo chown {item['owner']}:{item['group']} {dest}")
            if item.get('mode'):
                lines.append(f"sudo chmod {item['mode'][1:]} {dest}")
        lines.append('```')
        heading = f'Create needed folders for {name}'
        h1.children.append(Section(
            heading_line=f'## {heading}', heading_text=heading, level=2,
            content_lines=lines,
        ))

    if setup['firewall']:
        lines = ['', note, '', '```bash']
        for rule in setup['firewall']:
            if rule['allow_from'] == 'internal':
                lines.append(
                    f"sudo ufw allow from <internal-subnet> to any port "
                    f"{rule['port']} proto {rule['proto']} comment '{rule['comment']}'"
                )
            else:
                lines.append(
                    f"sudo ufw allow {rule['port']}/{rule['proto']} "
                    f"comment '{rule['comment']}'"
                )
        lines.append('```')
        heading = f'Open the firewall for {name}'
        h1.children.append(Section(
            heading_line=f'## {heading}', heading_text=heading, level=2,
            content_lines=lines,
        ))
...
def merge_setup_sections(sections, generated):
    """Place generated setup sections ahead of a container's own sections.

    A hand-written H2 with the same heading keeps its prose, which follows
    the generated commands. Unlike merge_sections, no lines are deduplicated,
    since that would drop repeated code fence lines.

    Args:
        sections:  Parsed stack-README.md sections for one container.
        generated: Sections from build_setup_sections().

    Returns:
        New list of sections; inputs are not modified.
    """
    result = deepcopy(sections)
    for gen_h1 in generated:
        h1 = next(
            (s for s in result if s.heading_text == gen_h1.heading_text), None
        )
        if h1 is None:
            result.append(deepcopy(gen_h1))
            continue

        ordered, used = [], set()
        for gen_h2 in gen_h1.children:
            h2 = next(
                (c for c in h1.children if c.heading_text == gen_h2.heading_text),
                None,
            )
            if h2 is None:
                ordered.append(deepcopy(gen_h2))
                continue
            prose = list(h2.content_lines)
            while prose and not prose[0].strip():
                prose.pop(0)
            while prose and not prose[-1].strip():
                prose.pop()
            h2.content_lines = list(gen_h2.content_lines)
            if prose:
                h2.content_lines += [''] + prose
            h2.content_lines.append('')
            ordered.append(h2)
            used.add(id(h2))
        h1.children = ordered + [c for c in h1.children if id(c) not in used]
    return result
