...
Every stack in `stacks/`, what it deploys, and where it runs. For the order hosts come up in, see [Bootstrap runbooks](README.md). For what a stack is made of mechanically, see [`scripts/project-layout.md`](../scripts/project-layout.md).
...
It needs live backends for monitoring (`ci01`), the auth chain (`id01`), and internal certs (`pk01`), so there is no point deploying it before those exist. Until then, [`traefik-bootstrap.md`](traefik-bootstrap.md) covers the temporary replacement. Do not run both on one VM: they fight over ports 80, 443, and 8443.
